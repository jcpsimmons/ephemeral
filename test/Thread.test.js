...
contract("Thread", ([deployer, poster, payer]) => {
    ...
    describe("posting", async () => {
        ...
        before(async () => {
            result = await thread.addReply(
                "This is the test reply!", { from: poster }
            );
        ...
        it("creates reply", async () => {
            ...
            assert.equal(event.content, "This is the test reply!", "content is correct");
            assert.equal(event.weiEarned, web3.utils.toWei('0', 'Ether'), "weiEarned is correct");
    ...
    describe("paying", async () => {
        it("pays OP", async () => {
            ...
            assert.equal(replyFundedEvent.args.to, poster, "poster (paid) addy is correct");
            assert.equal(replyFundedEvent.args.weiEarned, value * .95, "value paid is correct");

            assert.equal(transactionFeeEvent.args.to, '0xd501eD3C3626402FA5a381D844b54a265BFF0108', "my transaction fee addy is correct");
            assert.equal(transactionFeeEvent.args.weiEarned, value * .05, "transaction fee amt is correct");

            let payerBalanceAfter = await web3.eth.getBalance(payer);
            payerBalanceAfter = web3.utils.toWei(payerBalanceAfter, 'Ether');
            assert.isTrue(payerBalanceAfter < payerBalanceBefore, 'payer balance decreased');

            let posterBalanceAfter = await web3.eth.getBalance(poster);
            posterBalanceAfter = web3.utils.toWei(posterBalanceAfter, 'Ether');
            assert.isTrue(posterBalanceAfter > posterBalanceBefore, 'poster balance increased');
        })
    })
});
